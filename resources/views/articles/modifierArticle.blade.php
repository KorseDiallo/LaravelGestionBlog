...
<div class="container">
    <div class="row d-flex-justify-content-center align-item-center">
        <div class="col">
            <form action="/article/modifierArticle/{{$articles->id}}" method="post">
                @csrf
                <div class="form-group">
                    <label for="nomCategorie" class="form-label mt-4">Nom de l'Article</label>
                    <input type="text" name="titre" class="form-control" id="nomCategorie" placeholder="Article" value="{{$articles->titre}}">
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <select class="form-select" id="choix" name="categorie_id">
                        @foreach($categories as $categorie)
                            <option value="{{$categorie->id}}" {{$categorie->id==$articles->categorie_id ? 'selected': ''}}>{{$categorie->nomCategorie}}</option>
                        @endforeach
                    </select>
                </div>
